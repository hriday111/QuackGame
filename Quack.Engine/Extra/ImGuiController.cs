...
public unsafe class ImGuiController : IDisposable
{
    ...
    public ImGuiController(int width, int height, string? textureResource = null)
    {
        ...
        Shader = new Shader(
            ("Quack.Engine.Resources.Shaders.imgui.vert", ShaderType.VertexShader),
            ("Quack.Engine.Resources.Shaders.imgui.frag", ShaderType.FragmentShader));
    ...
    private void RecreateFontDeviceTexture(string? textureResource = null)
    {
        ...
        if (textureResource is not null)
        {
            ...
            if (stream is not null)
            {
                ...
                io.Fonts.AddFontFromMemoryTTF(fontData, dataLength, 40.0f, config, ranges.Data);
            }
        }
